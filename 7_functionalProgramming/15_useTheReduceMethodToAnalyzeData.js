...
//the callback function accpets four arguments
//the first argument is knwon as the accumulator
//which gets assigned the return value of the callback function from the previous iteration, the second is the current element being processed, the third is the index of that element and the fourth is the array upon which recude is called
//in addition to the callback function, reduce has an additional parameter which takes an initial value for the accumulator
//if this second parameter is not used, then the first iterator is skipped and the second iteration gets passed the first element of the array as the accumulator
...
const users = [
  ...
  console.log(usersObj); // returns { John: 34, Amy: 20, camperCat: 10 }
//example: the variable watchList holds an array of objects with information on several movies
...
//you may need to create other variables, and return the average ratings from getRating function
//note that the rating values are saved as strings in the object and need to be converteed into numbers before they are used in any mathematical operations
// The global variable
const watchList = [
    {
      "Title": "Inception",
